...
| **Stone plan** | **Tim Daw, *stonehenge-block-3d*, `data/locked_poses.json`** (commit `52e81ba`), 93 stones digitised from the M J Rees & Co 1989/90 survey of the monument, Historic England Archive sheet MP/STO0861, each graded by Daw's own `accuracy_class` | **CC BY-SA 4.0** — attribution given in the info view and in `Sources/HengeGeometry/Resources/stones/LICENSE-DATA.md`; share-alike honoured by publishing the derived CSV under the same licence (that file alone, not the app). Decision record: `docs/decisions/2026-09-23-stone-data-licences.md` | **In use** — `Sources/HengeGeometry/Resources/stones/daw-locked-poses.csv`, written by `scripts/import_daw_poses.py` from the pinned commit. Positions and yaw only for standing stones; footprints only for what lies. `seed_only` and `plan_digitised` rows ship as *provisional*. Daw's file does not cite Rees or Historic England; this repo does. Its height column is not used for standing stones; heights come from the Cleal row below |
| **Stone heights** | **Cleal, Walker & Montague, *Stonehenge in its Landscape: Twentieth-century Excavations* (EH Archaeological Report 10, 1995), Appendix 5 "Heights of Stones", pp 547–548**, compiled from the Chief Architect's Report of 1919 and Atkinson's records | Published on the Archaeology Data Service (eh_monographs_2014, mono 1089007) under the **ADS Terms of Use and Access**: reuse for historic-environment research including the commercial sector, with acknowledgement — given in the info view and here | **In use** — `Sources/HengeGeometry/Resources/stones/cleal-1995-appendix5.csv`, 142 rows transcribed by eye from the page images on 2026-09-23, feet and metres both kept, three printed disagreements flagged. Standing stones take the single figure; stones re-erected in 1958 (22, 57, 58) are recorded as they lay and get no standing height |
| **Stone heights, 1877** | **Petrie, *Stonehenge: Plans, Description, and Theories* (1880), "Details of the Stones", pp 9–12** — height above ground and level of top in inches for every stone he could measure in 1874–77 | Public domain | **In use** as the independent check only — `Sources/HengeGeometry/Resources/stones/petrie-1880-heights.csv`, transcribed from the Google Books scan on 2026-09-23. Pre-restoration, so it holds the only standing height of stone 22 in print; a test compares it with Cleal on the stones nothing moved between |
