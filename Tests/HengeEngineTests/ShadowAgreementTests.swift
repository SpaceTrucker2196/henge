...
final class RendererSetupTests: XCTestCase {
    ...
    func testUniformLayoutsAreTheExpectedSize() {
        // 8 matrices × 64 bytes + 20 float4s × 16 bytes; the float4s after
        // the camera/sun block are `cascadeRadii` (PCSS), `wind`, `grass`,
        // `night`, `season`, `shadowSource`, `haze` (the light shafts),
        // `torch`, `weatherState`, `milkyWay`, `viewport` (the render
        // scale), and `skyZenith`/`skyHorizon` (the ambient's two sky
        // constants); the eighth matrix is `worldToJ2000` for the star map.
        // This assertion has now caught four
        ...
        XCTAssertEqual(MemoryLayout<FrameUniforms>.size, 8 * 64 + 20 * 16)
