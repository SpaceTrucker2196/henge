...
final class RendererSetupTests: XCTestCase {
    ...
    func testUniformLayoutsAreTheExpectedSize() {
        // 8 matrices × 64 bytes + 18 float4s × 16 bytes; the float4s after
        // the camera/sun block are `cascadeRadii` (PCSS), `wind`, `grass`,
        // `night`, `season`, `shadowSource`, `haze` (the light shafts),
        // `torch`, `weatherState`, `milkyWay` and `viewport` (the render
        // scale); the eighth matrix is `worldToJ2000` for the star map.
        ...
        XCTAssertEqual(MemoryLayout<FrameUniforms>.size, 8 * 64 + 18 * 16)
